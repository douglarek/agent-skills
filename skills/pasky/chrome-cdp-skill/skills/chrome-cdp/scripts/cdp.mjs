...
function getWsUrl() {
  ...
  const linuxPath = resolve(homedir(), '.config/google-chrome/DevToolsActivePort');
  const macosPath = resolve(homedir(), 'Library/Application Support/Google/Chrome/DevToolsActivePort');
  let portFile;
  
  if (existsSync(linuxPath)) {
    portFile = linuxPath;
  } else if (existsSync(macosPath)) {
    portFile = macosPath;
  } else {
    throw new Error(`DevToolsActivePort not found. Chrome remote debugging may not be enabled.
    Check: chrome://inspect/#remote-debugging and toggle "Allow access to Chrome://inspect"
    Expected paths:
    - Linux: ${linuxPath}
    - macOS: ${macosPath}`);
  }
